...
from mavros_msgs.srv import SetMode, CommandBool, CommandTOL, CommandTOLRequest
...
def rc(pitch, roll, yaw, throttle):
    ...
    rc_msg.channels[3] = int(mapp(yaw, -100, 100, rc_min, rc_max))
    return rc_msg
...
def alt_callback(data):
    global current_x, current_y, euler_distance, bearing, current_z
...
def speed_callback(data):
    ...
    current_vx = data.twist.linear.x
    current_vy = data.twist.linear.y
...
def create_mpc():
    ...
    x = model.set_variable(var_type='_x', var_name='x')  # Position in x
    y = model.set_variable(var_type='_x', var_name='y')  # Position in y
    vx = model.set_variable(var_type='_x', var_name='vx')  # Velocity in x
    vy = model.set_variable(var_type='_x', var_name='vy')  # Velocity in y
    roll = model.set_variable(var_type='_x', var_name='roll')  # Roll angle
    pitch = model.set_variable(var_type='_x', var_name='pitch')  # Pitch angle
    yaw = model.set_variable(var_type='_x', var_name='yaw')  # Yaw angle
    p = model.set_variable(var_type='_x', var_name='p')  # Angular velocity in roll
    q = model.set_variable(var_type='_x', var_name='q')  # Angular velocity in pitch
    r = model.set_variable(var_type='_x', var_name='r')  # Angular velocity in yaw
    ...
    model.set_rhs('vx', vx)
    model.set_rhs('vy', vy)
    model.set_rhs('roll', p)
    model.set_rhs('pitch', q)
    model.set_rhs('yaw', r)
    model.set_rhs('p', q)
    model.set_rhs('q', p)
    model.set_rhs('r', p)
    ...
    Q = np.diag([40, 40, 1, 1, 40, 40, 40, 1, 1, 1])  # State weights
    R = np.diag([1.0, 1.0, 1.0])  # Input weights

    # Define cost function
    mterm = (model.x['x'] - x_set) ** 2 * Q[0, 0] + \
            (model.x['y'] - y_set) ** 2 * Q[1, 1] + \
            (model.x['vx'] - 0) ** 2 * Q[2, 2] + \
            (model.x['vy'] - 0) ** 2 * Q[3, 3] + \
            (model.x['roll'] - 0) ** 2 * Q[4, 4] + \
            (model.x['pitch'] - 0) ** 2 * Q[5, 5] + \
            (model.x['yaw'] - bearing) ** 2 * Q[6, 6] + \
            (model.x['p'] - 0) ** 2 * Q[7, 7] + \
            (model.x['q'] - 0) ** 2 * Q[8, 8] + \
            (model.x['r'] - 0) ** 2 * Q[9, 9]
    ...
    mpc.set_rterm(u2=R[0, 0], u3=R[1, 1], u4=R[2, 2])
    ...
    mpc.bounds['lower', '_u', 'u4'] = -100
    mpc.bounds['upper', '_u', 'u4'] = 100

    mpc.setup()

    # Set initial guess for control inputs
    mpc.x0 = np.zeros((10,))  # Initial state guess
    mpc.u0 = np.zeros((3,))   # Initial control input guess
    mpc.set_initial_guess()

    return mpc

def mpc_control(event):
    global mpc, current_x, current_y, current_yaw, euler_distance, bearing, x_set, y_set, current_vx, current_vy, current_roll, current_pitch, current_p, current_q, current_r

    # Update the current state vector with the latest values
    x0 = np.array([current_x, current_y,
    ...
    # Log current status
    rospy.loginfo(f"Yaw: {current_yaw} -- {bearing} | Distance: {euler_distance}")
    rospy.loginfo(f"Yaw rate: {yaw_rate} -- {current_r} || Pitch rate: {pitch_rate} -- {current_p} || Roll rate: {roll_rate} -- {current_q}")   
    throttle = pid(current_z)
    rc_msg = rc(pitch_rate, roll_rate, yaw_rate, throttle)
    pub.publish(rc_msg)
...
def start():
    ...
    arm()
    stab()
    rospy.init_node('pitch_roll_mpc', anonymous=True)
    ...
    rospy.Subscriber('/mavros/imu/data', Imu, angvel_callback)
